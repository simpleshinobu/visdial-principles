...
class DialogsReader(object):
    ...
    def __init__(self, dialogs_jsonpath: str, config):
        with open(dialogs_jsonpath, "r") as visdial_file:
            ...
            if not os.path.exists(self.tokens_path): ##generate tokens from sentence, and create tokens.json in "data/"
                save_dict = {}
                for key in ['questions', 'answers']:
                    save_key = key+"_"+self._split
                    if save_key not in save_dict.keys():
                        save_dict[save_key] = visdial_data["data"][key]
                        if key == 'questions':
                            for i in range(len(save_dict[save_key])):
                                save_dict[save_key][i] = word_tokenize(save_dict[save_key][i]+"?")
                        else:
                            for i in range(len(save_dict[save_key])):
                                save_dict[save_key][i] = word_tokenize(save_dict[save_key][i])
                with open(self.tokens_path, "w") as f:
                    json.dump(save_dict, f)
            if os.path.exists(self.tokens_path) and self._split == 'val2018':
                with open(self.tokens_path, 'r') as f:
                    save_dict = json.loads(f.read())
                    f.close()
                if 'questions_'+self._split not in save_dict.keys():
                    for key in ['questions', 'answers']:
                        save_key = key + "_" + self._split
                        if save_key not in save_dict.keys():
                            save_dict[save_key] = visdial_data["data"][key]
                    ...
                    with open(self.tokens_path, "w") as f:
                        json.dump(save_dict, f)
            if self._split in ['val2018', 'train']:
                print(self._split+'tokens load token from data/tokens.json')
                with open("data/tokens.json", 'r') as f:
                    data = json.loads(f.read())
                    f.close()
                self.questions = data['questions_'+self._split]
                self.answers = data['answers_' + self._split]
                # self.captions_wait = data['captions_' + self._split]
                self.questions.append("")
                self.answers.append("")
            else:
                self.questions = visdial_data["data"]["questions"]
                self.answers = visdial_data["data"]["answers"]  # list
    ...
    def __getitem__(self, image_id: int) -> Dict[str, Union[int, str, List]]:
        ...
        dialog_for_image = copy.deepcopy(self.dialogs[image_id]) ##change from copy -> deepcopy
