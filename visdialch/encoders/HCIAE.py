...
class HCIAE_Encoder(nn.Module):
    def __init__(self, config, vocabulary):
        ...
        self.fusion = nn.Linear(self.nhid * 2 + self.img_feature_size, self.nhid)
        ###cap att img
    ...
    def forward(self, batch):
        ...
        q_att_his_feat = self.ques_att_on_his(ques_feat, his_feat).view(batch_size, self.nhid)  # b 512

        q_att_img_feat = self.qh_att_on_img(ques_feat, q_att_his_feat, img).view(batch_size, -1)

        fused_vector = torch.cat((ques_feat, q_att_his_feat, q_att_img_feat), dim = -1)
