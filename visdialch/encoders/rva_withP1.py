...
class RvaEncoder_withP1(nn.Module):
    ...
    def forward(self, batch):
        ...
        fused_v = img_ans_feat.contiguous().view(-1, img_ans_feat.size(-1))
        fused_v = self.fusion_v(fused_v)
        fused_v = fused_v.contiguous().view(batch_size, num_rounds, fused_v.size(-1))
        fused_q = ques_encoded.contiguous().view(-1, ques_encoded.size(-1))
        fused_q = self.fusion_q(fused_q)
        fused_q = fused_q.contiguous().view(batch_size, num_rounds, fused_q.size(-1))

        fused_vector = fused_v * fused_q
        fused_vector = torch.cat((img_ans_feat, ques_encoded, fused_vector), -1)
