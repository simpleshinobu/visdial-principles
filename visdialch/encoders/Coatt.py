...
class CoAtt_Encoder(nn.Module):
    ...
    def forward(self, batch):
        ...
        img_atted_feat_f, img_att = self.qh_att_in_img(ques_atted_feat_f, his_atted_feat_f, img)
        img_atted_feat_f = img_atted_feat_f.view(batch_size, self.img_feature_size)

        fused_vector = torch.cat((ques_atted_feat_f, his_atted_feat_f, img_atted_feat_f), dim=-1)
